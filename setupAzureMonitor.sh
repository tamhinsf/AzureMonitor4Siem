...
AZ_STORAGE_ACCOUNT_BLOB_CONTAINER=mysiemsacontainer
...
az monitor log-profiles create --name "default" --location null --locations "global" \
    --categories "Delete" "Write" "Action"  --enabled true --days 7 \
    --service-bus-rule-id "/subscriptions/$AZ_SUBSCRIPTION_ID/resourceGroups/$AZ_RESOURCE_GROUP/providers/Microsoft.EventHub/namespaces/$AZ_EVENTHUB_NAMESPACE/authorizationrules/RootManageSharedAccessKey"
...
az eventhubs eventhub authorization-rule create --namespace-name $AZ_EVENTHUB_NAMESPACE \
    --resource-group $AZ_RESOURCE_GROUP --eventhub-name insights-operational-logs --rights Listen --name azureMonitor
